...
var liveblocks_default = getLiveblocks;

// types/canvas.ts
init_esm();
var CANVAS_NODE_TYPE = "canvasNode";
var CANVAS_EDGE_TYPE = "canvasEdge";
var NODE_SHAPES = [
  "rectangle",
  "diamond",
  "circle",
...
// trigger/design-agent.ts
var geminiActionSchema = external_exports.object({
...
var geminiResponseSchema = external_exports.object({
  actions: external_exports.array(geminiActionSchema),
  summary: external_exports.string()
});
var FILL_COLORS = NODE_COLORS.map((c) => c.fill);
var SYSTEM_PROMPT = `You are Ghost AI, an expert system design architect. Generate canvas actions to create a clear visual architecture diagram.
...
async function setAiPresence(liveblocks, roomId, thinking, ttl) {
...
var designAgent = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    const liveblocks = liveblocks_default();
    ...
    try {
      ...
      const { output: object } = await generateText({
        ...
        output: output_exports.object({ schema: geminiResponseSchema }),
      ...
      await liveblocks.mutateStorage(payload.roomId, ({ root }) => {
        ...
        for (const action of object.actions) {
          switch (action.type) {
            case "add_node": {
              const color = action.color && FILL_COLORS.includes(action.color) ? action.color : FILL_COLORS[1];
              const col = nodeIndex % 3;
              const row = Math.floor(nodeIndex / 3);
              const x = typeof action.x === "number" ? action.x : 100 + col * 250;
              const y = typeof action.y === "number" ? action.y : 100 + row * 150;
              nodeIndex++;
              nodes.set(
                action.id,
                new LiveObject({
            ...
            case "move_node": {
              ...
              if (node && typeof action.x === "number" && typeof action.y === "number") {
